...
void playerCheck::moveCheck(int order,int player=1)
{
	if (player < 0) {

	}
	while (!isbetween(m[player].move[order], choice) || comeonCheck(order) || doubleattackCheck(order) || provokeCheck(order)
		)
	{
		if (!isbetween(m[player].move[order], choice))
		{
			cout << "不在可选范围" << endl << "请重新输入";
			cin >> m[player].move[order];
		}
